...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            # downsampling
            nn.Conv2d(64, 128, 3, stride = 2, padding = 1), # channels: 64 -> 128
            nn.InstanceNorm2d(128),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(128, 256, 3, stride = 2, padding = 1), # channels: 128 -> 256
            nn.InstanceNorm2d(256),
            nn.LeakyReLU(inplace=True),           
            
            # khối transform 9 block 
            Residual(256),
            Residual(256),
            Residual(256),
            Residual(256),
            ...
            # upsampling
            nn.ConvTranspose2d(256, 128, 3, stride = 2, padding = 1), # channels: 256 -> 128
            nn.InstanceNorm2d(128),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(128, 64, 3, stride = 2, padding = 1), # channels: 256 -> 64
            nn.InstanceNorm2d(64),
            nn.ReLU(inplace=True), 
            
            # output
            nn.ReflectionPad2d(3),
            nn.Conv2d(64, 3, 4), # 64 -> 3 = input channels
            nn.Tanh()                       
        )
